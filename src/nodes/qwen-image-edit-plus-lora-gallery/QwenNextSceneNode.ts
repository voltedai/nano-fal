...
interface QwenNextSceneResponse {
  images: Array<{
    url: string
  }>
  seed: number
...
const nodeDefinition: NodeDefinition = {
  uid: 'fal-qwen-next-scene',
  name: 'Qwen Next Scene',
  category: 'Qwen / Gallery',
  version: '1.0.0',
  type: 'server',
  description: 'Create cinematic shot progressions and scene transitions',
  inputs: [
    {
      name: 'prompt',
      type: 'string',
      description: 'Describe the camera movement, framing change, or scene transition. Start with "Next Scene:" for best results'
    },
    {
      name: 'image',
      type: 'asset:image',
      description: 'Image to create the next scene from'
    }
  ],
  outputs: [
    {
      name: 'images',
      type: 'asset:image',
      description: 'Generated next scene images'
    },
    {
      name: 'seed',
      type: 'number',
...
const qwenNextSceneNode: NodeInstance = NanoSDK.registerNode(nodeDefinition)

qwenNextSceneNode.execute = async ({ inputs, parameters, context }) => {
  configureFalClient()
  ...
  try {
    ...
    const uploadedUrl = await uploadBufferToFal(buffer, format, { filenamePrefix: 'qwen-next-scene-input' })
    ...
    const strategy = createProgressStrategy({
      ...
      finalizingMessage: 'Finalizing next scene...',
    ...
    }) as unknown as QwenNextSceneResponse
    ...
    if (!images.length) {
      throw new Error('No images were returned by the Qwen Next Scene API')
    }
...
export default qwenNextSceneNode
