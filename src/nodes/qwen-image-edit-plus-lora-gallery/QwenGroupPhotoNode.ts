...
interface QwenGroupPhotoResponse {
  images: Array<{
    url: string
  }>
  seed: number
  ...
  inputs: [
    {
      name: 'prompt',
      type: 'string',
      description: 'Describe the group photo scene, setting, and style. The model will maintain character consistency and add vintage effects'
    },
    {
      name: 'image1',
      type: 'asset:image',
      description: 'First individual portrait image'
    },
    {
      name: 'image2',
      type: 'asset:image',
      description: 'Second individual portrait image'
    },
    {
      name: 'image3',
      type: 'asset:image',
      description: 'Optional third individual portrait image',
      optional: true
    },
    {
      name: 'image4',
      type: 'asset:image',
      description: 'Optional fourth individual portrait image',
      optional: true
    }
  ],
  outputs: [
    {
      name: 'images',
      type: 'asset:image',
      description: 'Generated group photo images'
    },
    {
      name: 'seed',
      type: 'number',
...
const qwenGroupPhotoNode: NodeInstance = NanoSDK.registerNode(nodeDefinition)

qwenGroupPhotoNode.execute = async ({ inputs, parameters, context }) => {
  configureFalClient()
  ...
  if (imageInputs.length < 2) {
    context.sendStatus({ type: 'error', message: 'At least 2 images are required' })
  ...
  try {
    ...
    for (let i = 0; i < imageInputs.length; i++) {
      const assetUri = imageInputs[i]
      const buffer: Buffer = await resolveAsset(assetUri, { asBuffer: true }) as Buffer
      const format = detectImageFormat(buffer)
      const uploadedUrl = await uploadBufferToFal(buffer, format, { filenamePrefix: `qwen-group-photo-${i + 1}` })
      imageUrls.push(uploadedUrl)

      context.sendStatus({
        type: 'running',
        message: `Uploaded image ${i + 1}/${imageInputs.length}`,
    ...
    const strategy = createProgressStrategy({
      ...
      finalizingMessage: 'Finalizing group photo...',
    ...
    const message = error?.message ?? 'Failed to create group photo'
    context.sendStatus({ type: 'error', message })
    throw error
  }
}

export default qwenGroupPhotoNode
